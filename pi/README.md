...
and `certify` commands from inside a [pi](https://pi.dev) session.
...
- `/validate <compiled-skill-dir> [flags...]` — runs `mellea-skills validate`
  (Step 7 lints + fixture smoke-check). Flags are passed through as-is; see
  `mellea-skills validate --help` (`--no-run`, `--all`).
- `/run <compiled-skill-dir> [flags...]` — runs `mellea-skills run` against
  a compiled pipeline. Flags are passed through as-is; see
...
- No cancellation: pi's per-command abort signal (`ctx.signal`) is only
  ...
  command handler — so a long-running `/compile`, `/validate`, `/run`, or
  `/certify` cannot currently be cancelled from within the extension. Use
  pi's own process-level interrupt if you need to stop one early.
